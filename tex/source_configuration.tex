\pgfplotstabletypeset[
    empty header,
    begin table=\begin{longtable},
    %every head row/.style={output empty row},
    every nth row={1}{before row=\hline},
    every first row/.append style={
        before row={%
            % Initial caption
            \caption{source\_configuration (NA)}
            \label{tab:DataTableSourceconfiguration}\\
            % Initial column headers
            \hline\hline             \multicolumn{1} { V{2 in}} { \textbf{\seqsplit{element\_name}}} & 
            \multicolumn{1} { V{0.8 in}} { \textbf{\seqsplit{type}}} & 
            \multicolumn{1} { V{2 in}} { \textbf{\seqsplit{external\_table}}} & 
            \multicolumn{1} { V{2 in} } {\textbf{\seqsplit{description}}} \\ \hline\hline \endfirsthead
            \multicolumn{4}{c}{Table \thetable\ source\_configuration (cont.)} \\
            % column headers on additional pages
            \hline\hline             \multicolumn{1} {V{2 in} } { \textbf{\seqsplit{element\_name}}} & 
            \multicolumn{1} {V{0.8 in} } { \textbf{\seqsplit{type}}} & 
            \multicolumn{1} {V{2 in} } { \textbf{\seqsplit{external\_table}}} & 
            \multicolumn{1} { V{2 in} } {\textbf{\seqsplit{description}}} \\ \hline\hline \endhead
            % Footer on 1st to penultimate pages
            \multicolumn{4}{r}{{Continued on next page}} \\
            \endfoot
            % Footer on last page of table
            \hline
            \multicolumn{4}{r}{{End of table}} \\ 
            \endlastfoot
        }
    },
    %
    end table=\end{longtable},
    col sep=tab,
    string type,
    columns/element_name/.style={
            string type, 
            column type= n{2 in}, 
            string replace*={_}{\_}
        },
    columns/type/.style={
            string type, 
            column type= V{0.8 in}, 
            string replace*={_}{\_}
        },
    columns/external_table/.style={
            string type, 
            column type= n{2 in}, 
            string replace*={_}{\_}
        },
    columns/description/.style={
            string type, 
            string replace*={_}{\_},
            column type = V{2 in}
        }
    ]{/Users/dyb/GitHub/C3S_311a_CDM/tables/source_configuration.csv}

\pgfplotstabletypeset[
    empty header,
    begin table=\begin{longtable},
    %every head row/.style={output empty row},
    every nth row={1}{before row=\hline},
    every first row/.append style={
        before row={%
            % Initial caption
            \caption{metadata\_source (NA)}
            \label{tab:DataTableMetadatasource}\\
            % Initial column headers
            \hline\hline             \multicolumn{1} { V{0.933333 in}} { \textbf{\seqsplit{metadata\_source}}} & 
            \multicolumn{1} { V{0.933333 in}} { \textbf{\seqsplit{description}}} & 
            \multicolumn{1} { V{0.933333 in}} { \textbf{\seqsplit{version}}} & 
            \multicolumn{1} { V{4.000000 in} } {\textbf{\seqsplit{URL}}} \\ \hline\hline \endfirsthead
            \multicolumn{4}{c}{Table \thetable\ metadata\_source (cont.)} \\
            % column headers on additional pages
            \hline\hline             \multicolumn{1} {V{0.933333 in} } { \textbf{\seqsplit{metadata\_source}}} & 
            \multicolumn{1} {V{0.933333 in} } { \textbf{\seqsplit{description}}} & 
            \multicolumn{1} {V{0.933333 in} } { \textbf{\seqsplit{version}}} & 
            \multicolumn{1} { V{4.000000 in} } {\textbf{\seqsplit{URL}}} \\ \hline\hline \endhead
            % Footer on 1st to penultimate pages
            \multicolumn{4}{r}{{Continued on next page}} \\
            \endfoot
            % Footer on last page of table
            \hline
            \multicolumn{4}{r}{{End of table}} \\ 
            \endlastfoot
        }
    },
    %
    end table=\end{longtable},
    col sep=tab,
    string type,
    columns/metadata_source/.style={
            string type, 
            column type= V{0.933333 in}, 
            string replace*={_}{\_}
        },
    columns/description/.style={
            string type, 
            column type= V{0.933333 in}, 
            string replace*={_}{\_}
        },
    columns/version/.style={
            string type, 
            column type= V{0.933333 in}, 
            string replace*={_}{\_}
        },
    columns/URL/.style={
            string type, 
            string replace*={_}{\_},
            column type = V{4.000000 in}
        }
    ]{/Users/dyb/GitHub/C3S_311a_CDM/tables/metadata_source.csv}
